var drawMainView = function() {
        ...
        self.dataset = []
        self.startTime = 0;
        self.totalGrapnNum = 0;
        self.singleWidth = self.width / 20; // 一个graph的宽度
        self.singleHeight = self.height / 20; //一个graph的高度
        window.setInterval(panGraph,1000); 
        ...
        function panGraph() {
            d3.selectAll('.sgraph')
                .each(function(d) {
                    var id = +d3.select(this).attr('id').split('sgraph')[1]
                    var move = self.totalGrapnNum - id + 1
                    d3.select(this)
                        .transition()
                        .duration(1000)
                        .attr('transform', 'translate(' + move * self.singleWidth + ',' + self.height / 2 + ')')
                       
                });
        }

        function drawSingleGraph(size_block, info, hash, data) {

            // panGraph();
            var nodes = [],
                edges = [];
            var startPoint = { x: 10, y: self.height / 2 }
            ...
            var g = self.vis.append('g')
                .attr('id', 'sgraph' + self.totalGrapnNum)
                .attr('class', 'sgraph')
                .attr('transform', 'translate(' + self.singleWidth / 2 + ',' + self.height / 2 + ')')
            self.totalGrapnNum += 1
            var force = self.forceLayout.nodes(nodes)
                .links(edges)
                .size([self.singleWidth, self.singleHeight]);
            ...
            var linkDistance = self.width / 10;
            force.alpha(0.1)
                .gravity(0.02)
                .friction(0.1)
                .charge(-80)
        ...
        function do_push(size_block, info, hash, data) {
            ...
            force.alpha(0.1)
                ...
                .start()
                // 删掉多余元素
            while (self.edgenum.length > 100) {

                for (var i = 0; i < self.nodenum[0]; i++) {
                    self.nodes.shift(); // 删掉数组第一个元素
                }
        ...
        function writeToScreen(message) {
            ...
            */
            ...
            drawSingleGraph(amount, info, hash, data)
